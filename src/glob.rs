...
pub(crate) static UNWANTED_A_MATCHER: Lazy<Matcher> = Lazy::new(|| Matcher::new(r#"a[href^="javascript:"]"#).unwrap());
pub(crate) static JSONLD_MATCHER: Lazy<Matcher> = Lazy::new(|| Matcher::new(r#"script[type="application/ld+json"]"#).unwrap());
...
pub(crate) static RX_SCHEMA_ORG: Lazy<Regex> =
    ...
    pub(crate) static RX_JSONLD_ARTICLE_TYPES: Lazy<Regex> =
    Lazy::new(|| Regex::new(r#"(?i)^Article|AdvertiserContentArticle|NewsArticle|AnalysisNewsArticle|AskPublicNewsArticle|BackgroundNewsArticle|OpinionNewsArticle|ReportageNewsArticle|ReviewNewsArticle|Report|SatiricalArticle|ScholarlyArticle|MedicalScholarlyArticle|SocialMediaPosting|BlogPosting|LiveBlogPosting|DiscussionForumPosting|TechArticle|APIReference$"#).unwrap());
